...
public class ReadyServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            {
                level = (String)param.nextElement();
                if (level.equals("firstlevel"))
                    select = (String)session.getAttribute("firstlevel");

                if (level.equals("secondlevel"))
                    select = (String)session.getAttribute("secondlevel");
 
                if (level.equals("thirdlevel"))
                    select = (String)session.getAttribute("thirdlevel");                
            }
            ...
            if (currentUser.isPaid())
            {
                ...
                status.setPackage(level,select,user_id);
                if (status.getQuestion() != null)
                {
                    status.setCorrect(0);
                    status.setCount(0);
